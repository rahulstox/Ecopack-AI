...
export class FreeAPIService {
  ...
  async testAPIs() {
    const results = {
      exchangeRates: { status: "testing", response: null, error: null },
      environmentalQuote: { status: "testing", response: null, error: null },
      marketData: { status: "testing", response: null, error: null },
      weatherImpact: { status: "testing", response: null, error: null },
    }

    // Test exchange rates
    try {
      results.exchangeRates.response = await this.getExchangeRates()
      results.exchangeRates.status = "success"
    } catch (error) {
      results.exchangeRates.status = "failed"
      results.exchangeRates.error = error.message
    }

    // Test environmental quote
    try {
      results.environmentalQuote.response = await this.getEnvironmentalQuote()
      results.environmentalQuote.status = "success"
    } catch (error) {
      results.environmentalQuote.status = "failed"
      results.environmentalQuote.error = error.message
    }

    // Test market data
    try {
      results.marketData.response = this.generateMarketData()
      results.marketData.status = "success"
    } catch (error) {
      results.marketData.status = "failed"
      results.marketData.error = error.message
    }

    // Test weather impact
    try {
      results.weatherImpact.response = await this.getWeatherImpact()
      results.weatherImpact.status = "success"
    } catch (error) {
      results.weatherImpact.status = "failed"
      results.weatherImpact.error = error.message
    }
